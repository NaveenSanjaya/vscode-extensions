...
export class SummarizationService {
     ...
     * @param targetTokenBudget - M03: On retry, instructs LLM to produce a shorter summary
    ...
    async summarize(
        ...
        // The Anthropic API requires the last message to be a 'user' role message.
        // If the conversation ends with an 'assistant' turn (which is common when
        // the agent has just finished responding), the API interprets the call as
        // "continue this assistant response" rather than "generate a fresh summary".
        // Result: the model emits ~3 whitespace tokens and stops (finishReason: stop).
        // Fix: always append a user message explicitly requesting the summary.
        ...
        // M03: On retry, append a token budget constraint to guide concise output.
        ...
        if (summaryMatch) {
            ...
            console.log(`[SummarizationService] Extracted summary (${extractedSummary.length} chars):\n${extractedSummary}`);
        ...
        if (withoutAnalysis.length > 50) {
            console.log(`[SummarizationService] Fallback summary (${withoutAnalysis.length} chars):\n${withoutAnalysis}`);
        ...
        throw new Error(
            'Summarization LLM did not return a <summary>...</summary> block. ' +
